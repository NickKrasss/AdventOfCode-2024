...
int day_5() {
    std::string file_path = "input5.txt";
    ...
    for (const auto& rule_line_raw : rule_lines) {
        ...
        try {
            ...
            std::cerr << "Неверный номер: " << rule_line << std::endl;
        }
    }
    ...
    std::cout << sum_middle_pages << std::endl;
    ...
    std::cout << sum_incorrect_middle_pages << std::endl;
